...
class ModsecurityAuditFormat < Fluent::Output
  ...
  def processSectionA(section) 
     ...
     matchData = section.match(/\[(?<modsec_timestamp>.+)\]\s(?<unique_id>.+)\s(?<src_ip>.+)\s(?<src_port>.+)\s(?<dst_ip>.+)\s(?<dst_port>.+)/)
  ...
  def processSectionB(section) 
     ...
     if section =~ /.*?\s\S+\s.+\n{1}/
       ...
       matchData = section.match(/(?<http_method>\S+)(?: +(?<requested_uri>[^ ]*) +(?<incoming_protocol>[\S]*))?/)
  ...
  def processSectionF(section) 
     ...
     if section =~ /.+/
       ...
       # line 1
       matchData = section.match(/(?<server_protocol>.+?)\s(?<response_code>[0-9]+)\s(?<response_status>.+)\n{1}/)
       hash = Hash[ matchData.names.zip(matchData.captures)]
